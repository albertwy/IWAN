...
def main(args):
	...
	for dataset_index in tqdm.trange(6):
		...
		for bert_context_reps, audio_context_reps, video_context_reps, bert_global_reps, audio_global_reps, video_global_reps, video_tensor, vlens, audio_tensor, alens, text_tensor, tlens, masks, v_masks, a_masks, labels in tqdm.tqdm(sarcasm_train):
			if torch.cuda.is_available():
				...
				output = final_model(bert_global_reps, audio_global_reps, video_global_reps, video_tensor, vlens, audio_tensor, alens, text_tensor, tlens, masks, v_masks, a_masks, get_feature=True)
		...
		for bert_context_reps, audio_context_reps, video_context_reps, bert_global_reps, audio_global_reps, video_global_reps, video_tensor, vlens, audio_tensor, alens, text_tensor, tlens, masks, v_masks, a_masks, labels in tqdm.tqdm(sarcasm_test):
			if torch.cuda.is_available():
				bert_global_reps = bert_global_reps.cuda()
				audio_global_reps = audio_global_reps.cuda()
				video_global_reps = video_global_reps.cuda()
