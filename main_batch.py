...
from models_batch import get_model
from dataset_batch import SarcasmDataset, collate_fn
...
def main(args):
	...
	for dataset_index in tqdm.trange(6):
		...
		for epoch in tqdm.trange(epochs):
			...
			for bert_global_reps, audio_global_reps, video_global_reps, video_tensor, vlens, audio_tensor, alens, text_tensor, tlens, masks, v_masks, a_masks, labels in tqdm.tqdm(sarcasm_train):
				if torch.cuda.is_available():
				...
				output = model(bert_global_reps, audio_global_reps, video_global_reps, video_tensor, vlens, audio_tensor, alens, text_tensor, tlens, masks, v_masks, a_masks)
		...
		for epoch in tqdm.trange(final_epochs):
			...
			for bert_global_reps, audio_global_reps, video_global_reps, video_tensor, vlens, audio_tensor, alens, text_tensor, tlens, masks, v_masks, a_masks, labels in tqdm.tqdm(sarcasm_train):
				...
				output = final_model(bert_global_reps, audio_global_reps, video_global_reps, video_tensor, vlens, audio_tensor, alens, text_tensor, tlens, masks, v_masks, a_masks)
				loss = loss_fn(output, labels)
		...
		avg_p, avg_r, avg_fscore, class_report = eval_prf(final_model, loss_fn, sarcasm_test, args)
		...
		results.append([avg_p, avg_r, avg_fscore])
...
def eval(model, loss_fn, sarcasm_dataset, args):
	...
	preds = []
	losses = []
	for bert_global_reps, audio_global_reps, video_global_reps, video_tensor, vlens, audio_tensor, alens, text_tensor, tlens, masks, v_masks, a_masks, labels in tqdm.tqdm(sarcasm_dataset):
		if torch.cuda.is_available():
			bert_global_reps = bert_global_reps.cuda()
			audio_global_reps = audio_global_reps.cuda()
			video_global_reps = video_global_reps.cuda()					
...
def eval_prf(model, loss_fn, sarcasm_dataset, args):
	model.eval()
	gds = []
	preds = []
	losses = []
	...
	avg_accuracy = round(accuracy_score(gds,preds)*100,2)
	avg_fscore = round(f1_score(gds,preds,average='weighted')*100,2)
	class_report = classification_report(gds,preds, digits=4, output_dict=True)
	class_report_print = classification_report(gds,preds, digits=4)

	return class_report["weighted avg"]["precision"], class_report["weighted avg"]["recall"], class_report["weighted avg"]["f1-score"], class_report_print
